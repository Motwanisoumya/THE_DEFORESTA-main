...
def convert_tiff_to_png(tiff_path, png_path):
    try:
        with rasterio.open(tiff_path) as src:
            band = src.read(1)  # Reads the first band; you can customize for RGB
            plt.imsave(png_path, band, cmap='viridis')  # Can use 'gray' or other colormaps
    ...
    try:
        ...
        for year in range(start_year, end_year + 1):
            ...
            if composite_path:
                ...
                png_path = composite_path.replace('.tiff', '.png')
                convert_tiff_to_png(composite_path, png_path)
            else:
